...
def handle(update):
    ...
    try:
        ...
        if uid in queue["occupied"]:
            ...
            if 'dice' in update:
                ...
                keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(
                    text="ɪɴsᴛᴀɢʀᴀᴍ", url="https://instagram.com/rianfirnandaa_")]])
        ...
        if text == "/test":
            ...
            bot.sendMessage(
                uid, "hai pengguna robo🙋‍♂️\nYuk Join My Grup @robogroup 😀", reply_markup=keyboard)
        ...
        if len(queue["free"]) > 1 and not uid in queue["occupied"]:
            ...
            if partner != uid:
                keyboard = ReplyKeyboardMarkup(keyboard=[
                    ["🎲", "🎯", "🎳", "🏀", "⚽", "🎰"], ['Next ▶️', '❌ Exit'], [
                        dict(text='Mutualan', request_contact=True)]
                ], resize_keyboard=True, one_time_keyboard=True)
