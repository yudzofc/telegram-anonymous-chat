...
if __name__ == '__main__':
	s = time.time()
	print('[#] Buatan\n[i] Created by Rian Firnanda Irsyadani\n')
	...
	try:
		...
		if uid in queue["occupied"]:
			...
			if 'dice' in update:
				...
				keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="ɪɴsᴛᴀɢʀᴀᴍ", url="https://instagram.com/rianfirnandaa_")]])
		...
		if text == "/start" or text == "/refresh":
			if not uid in queue["occupied"]:
				...
				keyboard = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="ɪɴsᴛᴀɢʀᴀᴍ", url="https://instagram.com/rianfirnandaa_"),InlineKeyboardButton(text="ɢʀᴜᴘ ᴄʜᴀᴛ", url="t.me/")]])
				bot.sendMessage(uid, "⚡️ROBO ANONYMOUS CHAT ⚡️\n\n_🇮🇩 Semoga Dapat teman atau jodoh\n🇳🇿 I hope you can make a friend or a partner\n\n?> untuk mencari teman obrolan gunakan perintah /search_", parse_mode='MarkDown', disable_web_page_preview=True , reply_markup=keyboard)
			...
			if not uid in queue["occupied"]:
				...
				for tiktok in trending:
					...
					inline = InlineKeyboardMarkup(inline_keyboard=[
						[InlineKeyboardButton(text="FOLLOW INSTAGRAM", url='https://instagram.com/rianfirnandaa_'), InlineKeyboardButton(text="GRUP CHAT", url="t.me/caritemanh")]])
					bot.sendMessage(uid1, f"LINK VIDEO = [DISINI]({link})\nUSERNAME TIKTOK = [DISINI](https://www.tiktok.com/@rianfirnanda)\nDESKRIPSI VIDEO ⬇️⬇️\n\n{descripsi}", parse_mode="Markdown", reply_markup=inline, reply_to_message_id=update['message_id'])
					time.sleep(2)
		...
		if len(queue["free"]) > 1 and not uid in queue["occupied"]:
			...
			if partner != uid:
				...
				bot.sendMessage(uid, '_🎈Pasangan kamu telah ditemukan, selamat menghalu wkwk_',parse_mode='MarkDown', reply_markup=keyboard)
				bot.sendMessage(partner, '_🎈Pasangan kamu telah ditemukann, selamat menghalu wkwk_',parse_mode='MarkDown', reply_markup=keyboard)
